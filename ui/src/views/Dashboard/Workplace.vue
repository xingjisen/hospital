...
import {ElCard, ElCol, ElDivider, ElRow, ElSkeleton} from 'element-plus'
import {useI18n} from '@/hooks/web/useI18n'
import {CountTo} from '@/components/CountTo'
import {WorkplaceTotal} from "@/api/dashboard/workplace/types";
...
const getCount = async () => {
    axios.request({
        url: "https://www.mxnzp.com/api/daily_word/recommend?count=1&app_id=wqnqfnoqrnp0tsoi&app_secret=StopcgLPrKsyBsa3RDG55yjqhGFELy2f",
        method: 'get'
    }).then(resp => {
        sentence.value = resp.data.data[0].content
    });
    axios.request({
        url: "https://api.istero.com/resource/weather/query?token=54c9e83991467ba4412804ff47158a9a&city=%E6%B5%8E%E5%8D%97",
        method: 'get'
    }).then(resp => {
        weather.value = resp.data.data
    });
    // const res = await getDetail(userStore.getUserInfo.)
}
/*
const getCount = async () => {
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                <div class="text-20px">
                                    ...
                                    }}，{{ userStore.getUserInfo.nickName }}，{{ t('workplace.happyDay') }}
                                </div>
                                <div style="color: #8c8c8c;font-size: 0.85rem">{{ sentence }}</div>
                                <div class="mt-10px text-14px text-gray-500">
                                    {{ weather.city }}今日{{ weather.weather }}，{{ weather.temperature }}℃ |
                                    风向: {{ weather.winddirection }} | 风力: {{ weather.windpower }}
