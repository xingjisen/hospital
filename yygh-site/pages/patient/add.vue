...
import { findByDictCode, findByParentId } from "@/api/dict";
import { add, getById, put } from "@/api/patient";
...
export default {
    ...
    return {
      ...
      props: {
        ...
        async lazyLoad(node, resolve) {
          const { level } = node;
          //异步获取省市区
          findByParentId(level ? node.value : "86").then(response => {
  ...
  methods: {
    ...
    saveData() {
      ...
      this.submitBnt = "正在提交...";
      add(this.patient).then(response => {
    ...
    updateData() {
      ...
      this.submitBnt = "正在提交...";
      put(this.patient).then(response => {
        this.$message.success("提交成功");
        window.location.href = "/patient";
      }).catch(e => {
        this.submitBnt = "保存";
