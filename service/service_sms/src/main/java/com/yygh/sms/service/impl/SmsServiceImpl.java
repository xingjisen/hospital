package com.yygh.sms.service.impl;
...
public class SmsServiceImpl implements SmsService {
    ...
    public boolean send(SmsVo smsVo) {
        // MQ 发送短信
        if (smsVo.getPhone() != null) {
            Map<String, Object> resp = sendTencentCode(smsVo.getPhone(), smsVo.getTemplateCode(), smsVo.getParam());
            if ("Ok".equals(resp.get("Code"))) {
                return true;
            }
        }
    ...
    private Map<String, Object> sendTencentCode(String phone, String templateCode, Map<String, Object> param) {
        templateCode = templateCode == null ? SmsConfig.TEMPLATE_ID : templateCode;
        Map<String, Object> map = new HashMap<>();
        try {
            // 实例化一个认证对象，入参需要传入腾讯云账户 SecretId 和 SecretKey，此处还需注意密钥对的保密
            // 代码泄露可能会导致 SecretId 和 SecretKey 泄露，并威胁账号下所有资源的安全性。以下代码示例仅供参考，建议采用更安全的方式来使用密钥，请参见：https://cloud.tencent.com/document/product/1278/85305
            ...
            String title = param.get("title").toString().split("\\|")[1];
            String reserveDate = param.get("reserveDate").toString();
            String body = name + " 您好，您成功预约" + title + "，日期为" + reserveDate + "，请注意规划时间。";
//            String[] templateParamSet1 = {body, ""};
